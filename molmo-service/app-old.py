...
model = AutoModelForCausalLM.from_pretrained(
    ...
    torch_dtype=torch.float16,        # (Warning says deprecated, but still accepted)
    device_map={"": 0},
    offload_folder=OFFLOAD_DIR_STR,
)
model.eval()

# Determine the model's dtype (fp16 on CUDA with torch_dtype="auto")
try:
    MODEL_DTYPE = next(model.parameters()).dtype
except Exception:
    MODEL_DTYPE = torch.float32  # safe fallback
...
def _normalize_images(key: str, value):
    ...
    IMPORTANT: move to EMBED_DEVICE **and** cast floating tensors to MODEL_DTYPE
               so inputs match the model's (often fp16 on CUDA).

    • If key == 'images':
        - Tensor 3D  [T, N, D] -> [1, T, N, D]
        - Tensor 4D  [B, T, N, D] -> as-is
        - List/Tuple of [T, N, D] -> stack on batch -> [B, T, N, D]
    • Else (pixel paths like 'pixel_values', 'image', 'image_values'):
        - Tensor 3D  [C, H, W]   -> [1, C, H, W]
        - Tensor 4D  [B, C, H, W] -> as-is
        - List/Tuple of CHW -> stack on batch
    """

    def _move_cast(x: torch.Tensor) -> torch.Tensor:
        # Move to device; if float, cast to model dtype (fp16/fp32)
    ...
    if key == "images":
        if isinstance(value, torch.Tensor):
            x = value
            if x.dim() == 3:           # [T, N, D] -> [1, T, N, D]
                x = x.unsqueeze(0)
            elif x.dim() >= 4:         # assume [B, T, N, D]
                pass
            else:
                raise TypeError(f"Unexpected dim for 'images': {tuple(x.shape)}")
            return _move_cast(x)
        ...
        for item in value:
            ...
            if t.dim() == 3:
                t = t.unsqueeze(0)
            tensors.append(_move_cast(t))
        return torch.cat(tensors, dim=0)  # [B, T, N, D]

    # Pixel path ('pixel_values' / 'image' / 'image_values')
    if isinstance(value, torch.Tensor):
        x = value
        if x.dim() == 3:               # [C, H, W] -> [1, C, H, W]
            x = x.unsqueeze(0)
        elif x.dim() >= 4:             # assume [B, C, H, W]
            pass
        else:
            raise TypeError(f"Unexpected dim for '{key}': {tuple(x.shape)}")
        return _move_cast(x)

    # list/tuple case for pixel path
    tensors = []
    for item in value:
        if not isinstance(item, torch.Tensor):
